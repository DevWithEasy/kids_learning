const banglaOkkor = {
    bangla: [
        {
            letter: 'অ',
            word: 'অযু',
            sentence: 'অযু করে মসজিদে যাই',
            vowel : true
        },
        {
            letter: 'আ',
            word: 'আযান',
            sentence: 'আযান দেয় মুয়াযযিন ভাই',
            vowel : true
        },
        {
            letter: 'ই',
            word: 'ইবাদত',
            sentence: 'ইবাদত কর অন্তর থেকে',
            vowel : true
        },
        {
            letter: 'ঈ',
            word: 'ঈদ',
            sentence: 'ঈদের আনন্দ চারদিকে',
            vowel : true
        },
        {
            letter: 'উ',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'ঊ',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'ঋ',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'এ',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'ঐ',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'ও',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'ঔ',
            word: '',
            sentence: '',
            vowel : true
        },
        {
            letter: 'ক',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'খ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'গ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঘ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঙ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'চ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ছ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'জ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঝ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঞ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ট',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঠ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ড',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঢ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ণ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ত',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'থ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'দ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ধ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ন',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'প',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ফ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ব',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ভ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ম',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'য',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'র',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ল',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'শ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ষ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'স',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'হ',
            word: '',
            sentence: '',
            vowel : false
        },
                {
            letter: 'ড়',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ঢ়',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'য়',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: 'ৎ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: ' ং',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: ' ঃ',
            word: '',
            sentence: '',
            vowel : false
        },
        {
            letter: ' ঁ',
            word: '',
            sentence: '',
            vowel : false
        },
    ],
    kar_bangla: [
        {
            letter: 'আ',
            kar: ' া',
            kar_letter: ['কা', 'খা', 'গা', 'ঘা', 'ঙা', 'চা', 'ছা', 'জা', 'ঝা', 'ঞা', 'টা', 'ঠা', 'ডা', 'ঢা', 'ণা', 'তা', 'থা', 'দা', 'ধা', 'না', 'পা', 'ফা', 'বা', 'ভা', 'মা', 'যা', 'রা', 'লা', 'শা', 'ষা', 'সা', 'হা', 'ড়া', 'ঢ়া', 'য়া'],
            kar_word: ['মা', 'বাবা', 'ঢাকা']
        },
        {
            letter: 'ই',
            kar: ' ি',
            kar_letter: ['কি', 'খি', 'গি', 'ঘি', 'ঙি', 'চি', 'ছি', 'জি', 'ঝি', 'ঞি', 'টি', 'ঠি', 'ডি', 'ঢি', 'ণি', 'তি', 'থি', 'দি', 'ধি', 'নি', 'পি', 'ফি', 'বি', 'ভি', 'মি', 'যি', 'রি', 'লি', 'শি', 'ষি', 'সি', 'হি', 'ড়ু', 'ঢ়ি', 'য়ি'],
            kar_word: ['কিনি', 'চিনি', 'মিনি']
        },
        {
            letter: 'ঈ',
            kar: ' ী',
            kar_letter: ['কী', 'খী', 'গী', 'ঘী', 'ঙী', 'চী', 'ছী', 'জী', 'ঝী', 'ঞী', 'টী', 'ঠী', 'ডী', 'ঢী', 'ণী', 'তী', 'থী', 'দী', 'ধী', 'নী', 'পী', 'ফী', 'বী', 'ভী', 'মী', 'যী', 'রী', 'লী', 'শী', 'ষী', 'সী', 'হী', 'ড়ী', 'ঢ়ী', 'য়ী'],
            kar_word: ['শশী', 'সীমানা', 'রীতি']
        },
        {
            letter: 'উ',
            kar: ' ু',
            kar_letter: ['কু', 'খু', 'ঘু', 'ঙু', 'চু', 'ছু', 'জু', 'ঝু', 'ঞু', 'টু', 'ঠু', 'ডু', 'ঢু', 'ণু', 'তু', 'থু', 'দু', 'ধু', 'নু', 'পু', 'ফু', 'বু', 'ভু', 'মু', 'যু', 'রু', 'লু', 'ষু', 'সু', 'ড়ু', 'ঢ়ু', 'য়ু'],
            kar_word: ['কুকুর', 'পুকুর', 'দুপুর']
        },
        {
            letter: 'ঊ',
            kar: ' ূ',
            kar_letter: ['কূ', 'খূ', 'গূ', 'ঘূ', 'ঙূ', 'চূ', 'ছূ', 'জূ', 'ঝূ', 'ঞূ', 'টূ', 'ঠূ', 'ডূ', 'ঢূ', 'ণূ', 'তূ', 'থূ', 'দূ', 'ধূ', 'নূ', 'পূ', 'ফূ', 'বূ', 'ভূ', 'মূ', 'যূ', 'লূ', 'ষূ', 'সূ', 'ড়ূ', 'ঢ়ূ', 'য়ূ', 'হূ'],
            kar_word: ['ভূত', 'মূল্য', 'সূচি']
        },
        {
            letter: 'ঋ',
            kar: ' ৃ',
            kar_letter: ['কৃ', 'খৃ', 'গৃ', 'ঘৃ', 'ঙৃ', 'চৃ', 'ছৃ', 'জৃ', 'ঝৃ', 'ঞৃ', 'টৃ', 'ঠৃ', 'ডৃ', 'ঢৃ', 'ণৃ', 'তৃ', 'থৃ', 'দৃ', 'ধৃ', 'নৃ', 'পৃ', 'ফৃ', 'বৃ', 'ভৃ', 'মৃ', 'যৃ', 'লৃ', 'ষৃ', 'সৃ', 'ড়ৃ', 'ঢ়ৃ', 'য়ৃ'],
            kar_word: ['কৃষক', 'তৃণ', 'পৃথিবী']
        },
        {
            letter: 'এ',
            kar: 'ে',
            kar_letter: ['কে', 'খে', 'গে', 'ঘে', 'ঙে', 'চে', 'ছে', 'জে', 'ঝে', 'ঞে', 'টে', 'ঠে', 'ডে', 'ঢে', 'ণে', 'তে', 'থে', 'দে', 'ধে', 'নে', 'পে', 'ফে', 'বে', 'ভে', 'মে', 'যে', 'রে', 'লে', 'শে', 'ষে', 'সে', 'হে', 'ড়ে', 'ঢ়ে', 'য়ে'],
            kar_word: ['চেয়ার', 'টেবিল', 'মেয়ে']
        },
        {
            letter: 'ঐ',
            kar: 'ৈ',
            kar_letter: ['কৈ', 'খৈ', 'গৈ', 'ঘৈ', 'ঙৈ', 'চৈ', 'ছৈ', 'জৈ', 'ঝৈ', 'ঞৈ', 'টৈ', 'ঠৈ', 'ডৈ', 'ঢৈ', 'ণৈ', 'তৈ', 'থৈ', 'দৈ', 'ধৈ', 'নৈ', 'পৈ', 'ফৈ', 'বৈ', 'ভৈ', 'মৈ', 'যৈ', 'রৈ', 'লৈ', 'শৈ', 'ষৈ', 'সৈ', 'হৈ', 'ড়ৈ', 'ঢ়ৈ', 'য়ৈ'],
            kar_word: ['তৈরি', 'বৈরী', 'নৈঋত']
        },
        {
            letter: 'ও',
            kar: ' ো',
            kar_letter: ['কো', 'খো', 'গো', 'ঘো', 'ঙো', 'চো', 'ছো', 'জো', 'ঝো', 'ঞো', 'টো', 'ঠো', 'ডো', 'ঢো', 'ণো', 'তো', 'থো', 'দো', 'ধো', 'নো', 'পো', 'ফো', 'বো', 'ভো', 'মো', 'যো', 'রো', 'লো', 'শো', 'ষো', 'সো', 'হো', 'ড়ো', 'ঢ়ো', 'য়ো'],
            kar_word: ['খোকা', 'পোকা', 'বোকা']
        },
        {
            letter: 'ঔ',
            kar: ' ৌ',
            kar_letter: ['কৌ', 'খৌ', 'গৌ', 'ঘৌ', 'ঙৌ', 'চৌ', 'ছৌ', 'জৌ', 'ঝৌ', 'ঞৌ', 'টৌ', 'ঠৌ', 'ডৌ', 'ঢৌ', 'ণৌ', 'তৌ', 'থৌ', 'দৌ', 'ধৌ', 'নৌ', 'পৌ', 'ফৌ', 'বৌ', 'ভৌ', 'মৌ', 'যৌ', 'রৌ', 'লৌ', 'শৌ', 'ষৌ', 'সৌ', 'হৌ', 'ড়ৌ', 'ঢ়ৌ', 'য়ৌ'],
            kar_word: ['নৌকা', 'মৌসুমি', 'পৌষ']
        },
    ],
    arabicborno: [

        {
            letter: 'ا',
            word: '',
            sentence: '',
            banglaPunctuation: 'আলিফ',
            position: {
                start: 'ا',
                center: 'ـا',
                end: 'ـا'
            }
        },
        {
            letter: 'ب',
            word: '',
            sentence: '',
            banglaPunctuation: 'বা',
            position: {
                start: 'بـ',
                center: 'ـبـ',
                end: 'ـب'
            }
        },
        {
            letter: 'ت',
            word: '',
            sentence: '',
            banglaPunctuation: 'তা',
            position: {
                start: 'تـ',
                center: 'ـت',
                end: 'ـت'
            }
        },
        {
            letter: 'ث',
            word: '',
            sentence: '',
            banglaPunctuation: 'ছা',
            position: {
                start: 'ثـ',
                center: 'ـث',
                end: 'ـث'
            }
        },
        {
            letter: 'ج',
            word: '',
            sentence: '',
            banglaPunctuation: 'জীম',
            position: {
                start: 'جـ',
                center: 'ـجـ',
                end: 'ـج'
            }
        },
        {
            letter: 'ح',
            word: '',
            sentence: '',
            banglaPunctuation: 'হা',
            position: {
                start: 'حـ',
                center: 'ـحـ',
                end: 'ـح'
            }
        },
        {
            letter: 'خ',
            word: '',
            sentence: '',
            banglaPunctuation: 'খ',
            position: {
                start: 'خـ',
                center: 'ـخـ',
                end: 'ـخ'
            }
        },
        {
            letter: 'د',
            word: '',
            sentence: '',
            banglaPunctuation: 'দাল',
            position: {
                start: 'د',
                center: 'ـد',
                end: 'ـد'
            }
        },
        {
            letter: 'ذ',
            word: '',
            sentence: '',
            banglaPunctuation: 'যাল',
            position: {
                start: 'ذ',
                center: 'ـذ',
                end: 'ـذ'
            }
        },
        {
            letter: 'ر',
            word: '',
            sentence: '',
            banglaPunctuation: 'র',
            position: {
                start: 'ر',
                center: 'ـر',
                end: 'ـر'
            }
        },
        {
            letter: 'ز',
            word: '',
            sentence: '',
            banglaPunctuation: 'জা',
            position: {
                start: 'ز',
                center: 'ـز',
                end: 'ـز'
            }
        },
        {
            letter: 'س',
            word: '',
            sentence: '',
            banglaPunctuation: 'সীন',
            position: {
                start: 'سـ',
                center: 'ـسـ',
                end: 'ـس'
            }
        },
        {
            letter: 'ش',
            word: '',
            sentence: '',
            banglaPunctuation: 'শীন',
            position: {
                start: 'شـ',
                center: 'ـشـ',
                end: 'ـش'
            }
        },
        {
            letter: 'ص	',
            word: '',
            sentence: '',
            banglaPunctuation: 'ছ্ব-দ',
            position: {
                start: 'صـ',
                center: 'ـصـ',
                end: 'ـص'
            }
        },

        {
            letter: 'ض',
            word: '',
            sentence: '',
            banglaPunctuation: 'দ্ব-দ',
            position: {
                start: 'ضـ',
                center: 'ـضـ',
                end: 'ـض'
            }
        },
        {
            letter: 'ط',
            word: '',
            sentence: '',
            banglaPunctuation: 'ত্ব',
            position: {
                start: 'طـ',
                center: 'ـطـ',
                end: 'ـط'
            }
        },
        {
            letter: 'ظ',
            word: '',
            sentence: '',
            banglaPunctuation: 'জ্ব',
            position: {
                start: 'ظـ',
                center: 'ـظـ',
                end: 'ـظ'
            }
        },
        {
            letter: 'ع',
            word: '',
            sentence: '',
            banglaPunctuation: 'আঈন',
            position: {
                start: 'عـ',
                center: 'ـعـ',
                end: 'ـع'
            }
        },

        {
            letter: 'غ',
            word: '',
            sentence: '',
            banglaPunctuation: 'গঈন',
            position: {
                start: 'غـ',
                center: 'ـغـ',
                end: 'ـغ'
            }
        },
        {
            letter: 'ف',
            word: '',
            sentence: '',
            banglaPunctuation: 'ফা',
            position: {
                start: 'فـ',
                center: 'ـفـ',
                end: 'ـف'
            }
        },
        {
            letter: 'ق',
            word: '',
            sentence: '',
            banglaPunctuation: 'ক্ব-ফ',
            position: {
                start: 'قـ',
                center: 'ـقـ',
                end: 'ـق'
            }
        },
        {
            letter: 'ك',
            word: '',
            sentence: '',
            banglaPunctuation: 'কা-ফ',
            position: {
                start: 'كـ',
                center: 'ك',
                end: 'ـك'
            }
        },
        {
            letter: 'ل',
            word: '',
            sentence: '',
            banglaPunctuation: 'লা-ম',
            position: {
                start: 'لـ',
                center: 'ـلـ',
                end: 'ـل'
            }
        },
        {
            letter: 'م',
            word: '',
            sentence: '',
            banglaPunctuation: 'মীম',
            position: {
                start: 'مـ',
                center: 'ـمـ',
                end: 'ـم'
            }
        },
        {
            letter: 'ن',
            word: '',
            sentence: '',
            banglaPunctuation: 'নূন',
            position: {
                start: 'نـ',
                center: 'ـنـ',
                end: 'ـن'
            }
        },
        {
            letter: 'ه',
            word: '',
            sentence: '',
            banglaPunctuation: 'হা',
            position: {
                start: 'هـ',
                center: 'ـهـ',
                end: 'ـه'
            }
        },
        {
            letter: 'و',
            word: '',
            sentence: '',
            banglaPunctuation: 'ওয়াও',
            position: {
                start: 'و',
                center: 'ـو',
                end: 'ـو'
            }
        },
        {
            letter: 'ء',
            word: '',
            sentence: '',
            banglaPunctuation: 'হাম্‌জাহ্',
            position: {
                start: 'ء',
                center: 'ء',
                end: 'ء'
            }
        },
        {
            letter: 'ى',
            word: '',
            sentence: '',
            banglaPunctuation: 'ইয়া',
            position: {
                start: 'يـ',
                center: 'ـيـ',
                end: 'ـي'
            }
        },
    ],
    englishborno: [
        {
            capital: 'A',
            small: 'a',
            word: '',
            sentence: '',
        },
        {
            capital: 'B',
            small: 'b',
            word: '',
            sentence: '',
        },
        {
            capital: 'C',
            small: 'c',
            word: '',
            sentence: '',
        },
        {
            capital: 'D',
            small: 'd',
            word: '',
            sentence: '',
        },
        {
            capital: 'E',
            small: 'e',
            word: '',
            sentence: '',
        },
        {
            capital: 'F',
            small: 'f',
            word: '',
            sentence: '',
        },
        {
            capital: 'G',
            small: 'g',
            word: '',
            sentence: '',
        },
        {
            capital: 'H',
            small: 'h',
            word: '',
            sentence: '',
        },
        {
            capital: 'I',
            small: 'i',
            word: '',
            sentence: '',
        },
        {
            capital: 'J',
            small: 'j',
            word: '',
            sentence: '',
        },
        {
            capital: 'K',
            small: 'k',
            word: '',
            sentence: '',
        },
        {
            capital: 'L',
            small: 'l',
            word: '',
            sentence: '',
        },
        {
            capital: 'M',
            small: 'm',
            word: '',
            sentence: '',
        },
        {
            capital: 'N',
            small: 'n',
            word: '',
            sentence: '',
        },
        {
            capital: 'O',
            small: 'o',
            word: '',
            sentence: '',
        },
        {
            capital: 'P',
            small: 'p',
            word: '',
            sentence: '',
        },
        {
            capital: 'Q',
            small: 'q',
            word: '',
            sentence: '',
        },
        {
            capital: 'R',
            small: 'r',
            word: '',
            sentence: '',
        },
        {
            capital: 'S',
            small: 's',
            word: '',
            sentence: '',
        },
        {
            capital: 'T',
            small: 't',
            word: '',
            sentence: '',
        },
        {
            capital: 'U',
            small: 'u',
            word: '',
            sentence: '',
        },
        {
            capital: 'V',
            small: 'v',
            word: '',
            sentence: '',
        },
        {
            capital: 'W',
            small: 'w',
            word: '',
            sentence: '',
        },
        {
            capital: 'X',
            small: 'x',
            word: '',
            sentence: '',
        },
        {
            capital: 'Y',
            small: 'y',
            word: '',
            sentence: '',
        },
        {
            capital: 'Z',
            small: 'z',
            word: '',
            sentence: '',
        }
    ],
    math_bangla: [
        {
            letter: '১',
            punctuation: 'এক',
        },
        {
            letter: '২',
            punctuation: 'দুই',

        },
        {
            letter: '৩',
            punctuation: 'তিন',
        },
        {
            letter: '৪',
            punctuation: 'চার',
        },
        {
            letter: '৫',
            punctuation: 'পাঁচ',
        },
        {
            letter: '৬',
            punctuation: 'ছয়',
        },
        {
            letter: '৭',
            punctuation: 'সাত',
        },
        {
            letter: '৮',
            punctuation: 'আট',
        },
        {
            letter: '৯',
            punctuation: 'নয়',
        },
        {
            letter: '১০',
            punctuation: 'দশ',
        },
        {
            letter: '১১',
            punctuation: 'এগার',
        },
        {
            letter: '১২',
            punctuation: 'বার',
        },
        {
            letter: '১৩',
            punctuation: 'তের',
        },
        {
            letter: '১৪',
            punctuation: 'চোদ্দ',
        },
        {
            letter: '১৫',
            punctuation: 'পনের',
        },
        {
            letter: '১৬',
            punctuation: 'ষোল',
        },
        {
            letter: '১৭',
            punctuation: 'সতের',
        },
        {
            letter: '১৮',
            punctuation: 'আঠার',
        },
        {
            letter: '১৯',
            punctuation: 'উনিশ',
        },
        {
            letter: '২০',
            punctuation: 'বিশ',
        },
        {
            letter: '২১',
            punctuation: 'একুশ',
        },
        {
            letter: '২২',
            punctuation: 'বাইশ',
        },
        {
            letter: '২৩',
            punctuation: 'তেইশ',
        },
        {
            letter: '২৪',
            punctuation: 'চব্বিশ',
        },
        {
            letter: '২৫',
            punctuation: 'পঁচিশ',
        },
        {
            letter: '২৬',
            punctuation: 'ছাব্বিশ',
        },
        {
            letter: '২৭',
            punctuation: 'সাতাইশ',
        },
        {
            letter: '২৮',
            punctuation: 'আটাইশ',
        },
        {
            letter: '২৯',
            punctuation: 'উনত্রিশ',
        },
        {
            letter: '৩০',
            punctuation: 'ত্রিশ',
        },
        {
            letter: '৩১',
            punctuation: 'একত্রিশ',
        },
        {
            letter: '৩২',
            punctuation: 'বত্রিশ',
        },
        {
            letter: '৩৩',
            punctuation: 'তেত্রিশ',
        },
        {
            letter: '৩৪',
            punctuation: 'চৌত্রিশ',
        },
        {
            letter: '৩৫',
            punctuation: 'পয়ত্রিশ',
        },
        {
            letter: '৩৬',
            punctuation: 'ছত্রিশ',
        },
        {
            letter: '৩৭',
            punctuation: 'সাইত্রিশ',
        },
        {
            letter: '৩৮',
            punctuation: 'আটত্রিশ',
        },
        {
            letter: '৩৯',
            punctuation: 'উনচল্লিশ',
        },
        {
            letter: '৪০',
            punctuation: 'চল্লিশ',
        },
        {
            letter: '৪১',
            punctuation: 'একচল্লিশ',
        },
        {
            letter: '৪২',
            punctuation: 'বিয়াল্লিশ',
        },
        {
            letter: '৪৩',
            punctuation: 'তেতাল্লিশ',
        },
        {
            letter: '৪৪',
            punctuation: 'চুয়াল্লিশ',
        },
        {
            letter: '৪৫',
            punctuation: 'পঁয়তাল্লিশ',
        },
        {
            letter: '৪৬',
            punctuation: 'ছেচল্লিশ',
        },
        {
            letter: '৪৭',
            punctuation: 'সাতচল্লিশ',
        },
        {
            letter: '৪৮',
            punctuation: 'আটচল্লিশ',
        },
        {
            letter: '৪৯',
            punctuation: 'ঊনপঞ্চাশ',
        },
        {
            letter: '৫০',
            punctuation: 'পঞ্চাশ',
        },
        {
            letter: '৫১',
            punctuation: 'একান্ন',
        },
        {
            letter: '৫২',
            punctuation: 'বাহান্ন',
        },
        {
            letter: '৫৩',
            punctuation: 'তিপ্পান্ন',
        },
        {
            letter: '৫৪',
            punctuation: 'চুয়ান্ন',
        },
        {
            letter: '৫৫',
            punctuation: 'পঞ্চান্ন',
        },
        {
            letter: '৫৬',
            punctuation: 'ছাপ্পান্ন',
        },
        {
            letter: '৫৭',
            punctuation: 'সাতান্ন',
        },
        {
            letter: '৫৮',
            punctuation: 'আটান্ন',
        },
        {
            letter: '৫৯',
            punctuation: 'ঊনষাট',
        },
        {
            letter: '৬০',
            punctuation: 'ষাট',
        },
        {
            letter: '৬১',
            punctuation: 'একষট্টি',
        },
        {
            letter: '৬২',
            punctuation: 'বাষট্টি',
        },
        {
            letter: '৬৩',
            punctuation: 'তেষট্টি',
        },
        {
            letter: '৬৪',
            punctuation: 'চৌষট্টি',
        },
        {
            letter: '৬৫',
            punctuation: 'পঁয়ষট্টি',
        },
        {
            letter: '৬৬',
            punctuation: 'ছেষট্টি',
        },
        {
            letter: '৬৭',
            punctuation: 'সাতষট্টি',
        },
        {
            letter: '৬৮',
            punctuation: 'আটষট্টি',
        },
        {
            letter: '৬৯',
            punctuation: 'ঊনসত্তর',
        },
        {
            letter: '৭০',
            punctuation: 'সত্তর',
        },
        {
            letter: '৭১',
            punctuation: 'একাত্তর',
        },
        {
            letter: '৭২',
            punctuation: 'বাহাত্তর',
        },
        {
            letter: '৭৩',
            punctuation: 'তিয়াত্তর',
        },
        {
            letter: '৭৪',
            punctuation: 'চুয়াত্তর',
        },
        {
            letter: '৭৫',
            punctuation: 'পঁচাত্তর',
        },
        {
            letter: '৭৬',
            punctuation: 'ছিয়াত্তর',
        },
        {
            letter: '৭৭',
            punctuation: 'সাতাত্তর',
        },
        {
            letter: '৭৮',
            punctuation: 'আটাত্তর',
        },
        {
            letter: '৭৯',
            punctuation: 'ঊনআশি',
        },
        {
            letter: '৮০',
            punctuation: 'আশি',
        },
        {
            letter: '৮১',
            punctuation: 'একাশি',
        },
        {
            letter: '৮২',
            punctuation: 'বিরাশি',
        },
        {
            letter: '৮৩',
            punctuation: 'তিরাশি',
        },
        {
            letter: '৮৪',
            punctuation: 'চুরাশি',
        },
        {
            letter: '৮৫',
            punctuation: 'পঁচাশি',
        },
        {
            letter: '৮৬',
            punctuation: 'ছিয়াশি',
        },
        {
            letter: '৮৭',
            punctuation: 'সাতাশি',
        },
        {
            letter: '৮৮',
            punctuation: 'অষ্টআশি',
        },
        {
            letter: '৮৯',
            punctuation: 'ঊননব্বই',
        },
        {
            letter: '৯০',
            punctuation: 'নব্বই',
        },
        {
            letter: '৯১',
            punctuation: 'একানব্বই',
        },
        {
            letter: '৯২',
            punctuation: 'বিরানব্বই',
        },
        {
            letter: '৯৩',
            punctuation: 'তিরানব্বই',
        },
        {
            letter: '৯৪',
            punctuation: 'চুরানব্বই',
        },
        {
            letter: '৯৫',
            punctuation: 'পঁচানব্বই',
        },
        {
            letter: '৯৬',
            punctuation: 'ছিয়ানব্বই',
        },
        {
            letter: '৯৭',
            punctuation: 'সাতানব্বই',
        },
        {
            letter: '৯৮',
            punctuation: 'আটানব্বই',
        },
        {
            letter: '৯৯',
            punctuation: 'নিরানব্বই',
        },
        {
            letter: '১০০',
            punctuation: 'একশ',
        }
    ],
    math_english: [
        {
            letter: '0',
            punctuation: 'Zero',
        },
        {
            letter: '1',
            punctuation: 'One',
        },
        {
            letter: '2',
            punctuation: 'Two',
        },
        {
            letter: '3',
            punctuation: 'Three',
        },
        {
            letter: '4',
            punctuation: 'Four',
            lang: 'EN'
        },
        {
            letter: '5',
            punctuation: 'Five',
        },
        {
            letter: '6',
            punctuation: 'Six',
        },
        {
            letter: '7',
            punctuation: 'Seven',
        },
        {
            letter: '8',
            punctuation: 'Eight',
        },
        {
            letter: '9',
            punctuation: 'Nine',
        },
        {
            letter: '10',
            punctuation: 'Ten',
        },
        {
            letter: '11',
            punctuation: 'eleven',
        },
        {
            letter: '12',
            punctuation: 'twelve',
        },
        {
            letter: '13',
            punctuation: 'thirteen',
        },
        {
            letter: '14',
            punctuation: 'fourteen',
        },
        {
            letter: '15',
            punctuation: 'fifteen',
        },
        {
            letter: '16',
            punctuation: 'sixteen',
        },
        {
            letter: '17',
            punctuation: 'seventeen',
        },
        {
            letter: '18',
            punctuation: 'eighteen',
        },
        {
            letter: '19',
            punctuation: 'nineteen',
        },
        {
            letter: '20',
            punctuation: 'Twenty ',
        },
        {
            letter: '21',
            punctuation: 'Twenty One',
        },
        {
            letter: '22',
            punctuation: 'Twenty ',
        },
        {
            letter: '23',
            punctuation: 'Twenty ',
        },
        {
            letter: '24',
            punctuation: 'Twenty Four',
        },
        {
            letter: '25',
            punctuation: 'Twenty Five',
        },
        {
            letter: '26',
            punctuation: 'Twenty Six',
        },
        {
            letter: '27',
            punctuation: 'Twenty Seven',
        },
        {
            letter: '28',
            punctuation: 'Twenty Eight',
        },
        {
            letter: '29',
            punctuation: 'Twenty Nine',
        },
        {
            letter: '30',
            punctuation: 'Thirty ',
        },
        {
            letter: '31',
            punctuation: 'Thirty One',
        },
        {
            letter: '32',
            punctuation: 'Thirty Two',
        },
        {
            letter: '33',
            punctuation: 'Thirty Three',
        },
        {
            letter: '34',
            punctuation: 'Thirty Four',
        },
        {
            letter: '35',
            punctuation: 'Thirty Five',
        },
        {
            letter: '36',
            punctuation: 'Thirty Six',
        },
        {
            letter: '37',
            punctuation: 'Thirty Seven',
        },
        {
            letter: '38',
            punctuation: 'Thirty Eight',
        },
        {
            letter: '39',
            punctuation: 'Thirty Nine',
        },
        {
            letter: '40',
            punctuation: 'Fourty ',
        },
        {
            letter: '41',
            punctuation: 'Fourty One',
        },
        {
            letter: '42',
            punctuation: 'Fourty Two',
        },
        {
            letter: '43',
            punctuation: 'Fourty Three',
        },
        {
            letter: '44',
            punctuation: 'Fourty Four',
        },
        {
            letter: '45',
            punctuation: 'Fourty Five',
        },
        {
            letter: '46',
            punctuation: 'Fourty Six',
        },
        {
            letter: '47',
            punctuation: 'Fourty Seven',
        },
        {
            letter: '48',
            punctuation: 'Fourty Eight',
        },
        {
            letter: '49',
            punctuation: 'Fourty Nine',
        },
        {
            letter: '50',
            punctuation: 'Fifety ',
        },
        {
            letter: '51',
            punctuation: 'Fifety One',
        },
        {
            letter: '52',
            punctuation: 'Fifety Two',
        },
        {
            letter: '53',
            punctuation: 'Fifety Three',
        },
        {
            letter: '54',
            punctuation: 'Fifety Four',
        },
        {
            letter: '55',
            punctuation: 'Fifety Five',
        },
        {
            letter: '56',
            punctuation: 'Fifety Six',
        },
        {
            letter: '57',
            punctuation: 'Fifety Seven',
        },
        {
            letter: '58',
            punctuation: 'Fifety Eight',
        },
        {
            letter: '59',
            punctuation: 'Fifety Nine',
        },
        {
            letter: '60',
            punctuation: 'Sixty ',
        },
        {
            letter: '61',
            punctuation: 'Sixty One',
        },
        {
            letter: '62',
            punctuation: 'Sixty Two',
        },
        {
            letter: '63',
            punctuation: 'Sixty Three',
        },
        {
            letter: '64',
            punctuation: 'Sixty Four',
        },
        {
            letter: '65',
            punctuation: 'Sixty Five',
        },
        {
            letter: '66',
            punctuation: 'Sixty Six',
        },
        {
            letter: '67',
            punctuation: 'Sixty Seven',
        },
        {
            letter: '68',
            punctuation: 'Sixty Eight',
        },
        {
            letter: '69',
            punctuation: 'Sixty Nine',
        },
        {
            letter: '70',
            punctuation: 'Seventy ',
        },
        {
            letter: '71',
            punctuation: 'Seventy One',
        },
        {
            letter: '72',
            punctuation: 'Seventy Two',
        },
        {
            letter: '73',
            punctuation: 'Seventy Three',
        },
        {
            letter: '74',
            punctuation: 'Seventy Four',
        },
        {
            letter: '75',
            punctuation: 'Seventy Five',
        },
        {
            letter: '76',
            punctuation: 'Seventy Six',
        },
        {
            letter: '77',
            punctuation: 'Seventy Seven',
        },
        {
            letter: '78',
            punctuation: 'Seventy Eight',
        },
        {
            letter: '79',
            punctuation: 'Seventy Nine',
        },
        {
            letter: '80',
            punctuation: 'Eighty ',
        },
        {
            letter: '81',
            punctuation: 'Eighty One',
        },
        {
            letter: '82',
            punctuation: 'Eighty Two',
        },
        {
            letter: '83',
            punctuation: 'Eighty Three',
        },
        {
            letter: '84',
            punctuation: 'Eighty Four',
        },
        {
            letter: '85',
            punctuation: 'Eighty Five',
        },
        {
            letter: '86',
            punctuation: 'Eighty Six',
        },
        {
            letter: '87',
            punctuation: 'Eighty Seven',
        },
        {
            letter: '88',
            punctuation: 'Eighty Eight',
        },
        {
            letter: '89',
            punctuation: 'Eighty Nine',
        },
        {
            letter: '90',
            punctuation: 'Ninety ',
        },
        {
            letter: '91',
            punctuation: 'Ninety One',
        },
        {
            letter: '92',
            punctuation: 'Ninety Two',
        },
        {
            letter: '93',
            punctuation: 'Ninety Three',
        },
        {
            letter: '94',
            punctuation: 'Ninety Four',
        },
        {
            letter: '95',
            punctuation: 'Ninety Five',
        },
        {
            letter: '96',
            punctuation: 'Ninety Six',
        },
        {
            letter: '97',
            punctuation: 'Ninety Seven',
        },
        {
            letter: '98',
            punctuation: 'Ninety Eight',
        },
        {
            letter: '99',
            punctuation: 'Ninety Nine',
        },
        {
            letter: '100',
            punctuation: 'One Hundred',
        }
    ],
    day_bangla: [
        {
            name : 'শনিবার',
            audio : ''
        },
        {
            name : 'রবিবার',
            audio : ''
        },
        {
            name : 'সোমবার',
            audio : ''
        },
        {
            name : 'মঙ্গলবার',
            audio : ''
        },
        {
            name : 'বুধবার',
            audio : ''
        },
        {
            name : 'বৃহস্পতিবার',
            audio : ''
        },
        {
            name : 'শুক্রবার',
            audio : ''
        },
    ],
    day_english: [
        {
            name : 'Saturday',
            audio : ''
        },
        {
            name : 'Sunday',
            audio : ''
        },
        {
            name : 'Monday',
            audio : ''
        },
        {
            name : 'Tuesday',
            audio : ''
        },
        {
            name : 'Wednesday',
            audio : ''
        },
        {
            name : 'Thursday',
            audio : ''
        },
        {
            name : 'Friday',
            audio : ''
        },
    ],
    month_bangla: [
        {
            name : 'বৈশাখ',
            punctuation : ''
        },
        {
            name : 'জ্যৈষ্ঠ',
            punctuation : ''
        },
        {
            name : 'আষাঢ়',
            punctuation : ''
        },
        {
            name : 'শ্রাবণ',
            punctuation : ''
        },
        {
            name : 'ভাদ্র',
            punctuation : ''
        },
        {
            name : 'আশ্বিন',
            punctuation : ''
        },
        {
            name : 'কার্তিক',
            punctuation : ''
        },
        {
            name : 'অগ্রহায়ণ',
            punctuation : ''
        },
        {
            name : 'পৌষ',
            punctuation : ''
        },
        {
            name : 'মাঘ',
            punctuation : ''
        },
        {
            name : 'ফাল্গুন',
            punctuation : ''
        },
        {
            name : 'চৈত্র',
            punctuation : ''
        }
    ],
    month_english: [
        {
            name : 'January',
            punctuation : 'জানুয়ারি'
        },
        {
            name : 'February',
            punctuation : 'ফেব্রুয়ারী'
        },
        {
            name : 'March',
            punctuation : 'মার্চ'
        },
        {
            name : 'April',
            punctuation : 'এপ্রিল'
        },
        {
            name : 'May',
            punctuation : 'মে'
        },
        {
            name : 'June',
            punctuation : 'জুন'
        },
        {
            name : 'July',
            punctuation : 'জুলাই'
        },
        {
            name : 'August',
            punctuation : 'আগস্ট'
        },
        {
            name : 'September',
            punctuation : 'সেপ্টেম্বর'
        },
        {
            name : 'October',
            punctuation : 'অক্টোবর'
        },
        {
            name : 'November',
            punctuation : 'নভেম্বর'
        },
        {
            name : 'December',
            punctuation : 'ডিসেম্বর'
        }
    ],
    season_bangla : [
        {
            name : 'গ্রীষ্মকাল',
            audio : ''
        },
        {
            name : 'বর্ষাকাল',
            audio : ''
        },
        {
            name : 'শরৎকাল',
            audio : ''
        },
        {
            name : 'হেমন্তকাল',
            audio : ''
        },
        {
            name : 'শীতকাল',
            audio : ''
        },
        {
            name : 'বসন্তকাল',
            audio : ''
        }
    ],
    season_english : [
        {
            name : 'Summer',
            audio : ''
        },
        {
            name : 'Rainy',
            audio : ''
        },
        {
            name : 'Autumn',
            audio : ''
        },
        {
            name : 'Late Autumn',
            audio : ''
        },
        {
            name : 'Winter',
            audio : ''
        },
        {
            name : 'Spring',
            audio : ''
        }
    ]
}

module.exports = banglaOkkor